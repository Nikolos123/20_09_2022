...
class BookCustomViewSet(ListModelMixin,DestroyModelMixin,RetrieveModelMixin,GenericViewSet):
    queryset =  Book.objects.all()
    serializer_class =  BookModelSerializer
    renderer_classes = [JSONRenderer, BrowsableAPIRenderer]
...
#        return Book.objects.filter(name__contains='boo1')#содержит
# #
#
class BookListAPIView(ListAPIView):
   serializer_class = BookModelSerializer
...
class FilterBookModelViewSet(ModelViewSet):
   queryset = Book.objects.all()
   serializer_class = BookModelSerializer
...
class BookDjangoFilterViewSet(ModelViewSet):
   queryset = Book.objects.all()
   serializer_class = BookModelSerializer
   # filterset_fields = ['id','name','authors']
